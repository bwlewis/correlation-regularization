...
example https://en.wikipedia.org/wiki/Partial_correlation and
...
sample correlation matrix are equally likely to occur in any direction from any
projection of our data (that the errors are white noise, basically).  As we can
see above, most of the information is concentrated in projections corresponding
to the first few eigenvectors.  But white  noise will have information content
in all the projected dimensions more or less equally.
...
correlation information. Most importantly, we're cutting off the projected
subspace associated tiny eigenvalues, avoiding the problem of division by tiny
...
regularization operators (so-called shrinkage targets), and can apply more
broadly than the simple approach taken in this note.
...
browsable(div(list(HTML("<center>"), tags$h3("Connectivity threshold (adjust slider to change)"), slider, vis, relay)))
```
