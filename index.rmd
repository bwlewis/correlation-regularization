...
Now that we have a universe of stocks with valid price data, convert
those prices to log(returns) for the remaining analysis.
...
is, matrix inversion tends to blow the noise up.

Think of the overall goal of this example as a model: given the input of a
bunch of historical stock prices, produce the output of a network that
highlights associations within and between the groups of stocks.

Ledoit and Wolf's sensible answer to reducing the influence of noise is
_regularization_. Regularization replaces models with _different_, but related,
...
of the total information in the correlation matrix is contained in the first
few eigenvectors.
...
(https://cran.r-project.org/package=corpcor) by Juliane Schafer, Rainer
...
precision matrix P, and you should try it! The result is similar to what we get
from our simple truncated eigenvalue decomposition regularization.
