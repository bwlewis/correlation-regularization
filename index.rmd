...
far from ground-breaking: you already can mostly guess the output. Stocks get
grouped together into pretty obvious business sectors.
...
regularization approach based on a matrix eigenvalue decomposition, but it's
...
Poor's 500 index at the time of this writing. The code downloads daily adjusted
closing prices from 2012 until the present. Modify this code to experiment with
...
still missing values, just remove those symbols that contain them, possibly
further reducing the universe of stock symbols we're working with.
...
prices to log(returns) for the remaining analysis (where by returns I mean
simply the ratio of prices relative to the first price).
...
A Nobel-prize winning approach to dealing with the second problem is to
consider cointegration between series instead of correlation, see for example
notes by Eric Zivot
...
Wikipedia (https://en.wikipedia.org/wiki/Cointegration).  I also have some of
my own weird technical notes on the numerics of cointegration
(http://illposed.net/cointegration.html).  Cointegration is a wonderful, but
deep topic. Instead, let's try a lesser but simpler approach.

We can try to address at least part of issue 2 above by controlling for
confounding variables, at least partially.  One approach considers _partial
correlation_ instead of correlation (see for example
https://en.wikipedia.org/wiki/Partial_correlation and
https://www.statlect.com/glossary/precision-matrix). That approach works best
in practice with approximately normal data--one reason for the switch to
log(returns) instead of prices. We will treat the entries of the precision
matrix as measures of association in a network of stocks below.

It's worth stating that approach basically treats the log(returns) series as a
bunch of vectors and not so much bona fide time series, and can't handle as
many pathologies that might occur as well as cointegration can. But as we will
see, this simple technique is still pretty effective at finding structure in
our data. (And indeed related methods as discussed by Ledoit and Wolf and
...
the entries of the inverse of the correlation matrix.
...
amount noise can be hugely amplified if we invert the correlation matrix.
That's because, as we will soon see, the correlation matrix contains tiny
eigenvalues and matrix inversion effectively divides the noise by those tiny
values.  Simply stated, dividing by a tiny number returns a big number--that
is, matrix inversion tends to blow the noise up. This is a fundamental issue
(in a sense, _the_ fundamental issue) common to many inverse problems.

Think of our overall goal as a model: given the input of a bunch of historical
stock prices, produce the output of a network that highlights associations
within and between the groups of stocks. Stated like this, our model is a kind
of inverse problem--one that involves finding a precision matrix.
...
structured estimate of the covariance. We will use a similar but simpler kind
of regularization based on an eigenvalue decomposition of the correlation
matrix `X`.
...
projection of the correlation matrix into a plane (2-d), and so on. The
eigenvalues estimate the proportion of information (or variability if you
prefer) from the original correlation matrix contained in each eigenvector.
...
of the information in the correlation matrix is contained in the first
few eigenvectors.

Let's assume, perhaps unreasonably, that the errors in our estimate of the
sample correlation matrix are equally likely to occur in any direction (that
the errors are white noise, basically).  As we can see above, most of the
...
correlation information. Most importantly, we're cutting off the subspace
...
correlation matrix (the precision matrix).
...
(http://epubs.siam.org/doi/book/10.1137/1.9780898719697) for
...
This gives us another heuristic way to choose a projection dimension, at least
if we have an idea about the number of clusters to look for.  See
https://bwlewis.github.io/crosstool/pca.html for an unfinished example that is
related to this one that focuses on clustering and k-means of genomic data.



## A precision matrix, finally

Finally, we form the precision matrix `P` from the regularized correlation
...
oddly the inclusion of CAT in this list), and so on.
With the threshold value of 0.9 above, a few stocks aren't connected to any
others -- they appear in gray.
...
# Crosstalk shared data sources
sd1 = SharedData$new(data.frame(key=paste(seq(0, length(threshold) - 1))), key=~key)
sd2 = SharedData$new(data.frame(1:length(threshold)))
# Relay the slider values directly to the graph
relay = crosstool(sd1, "transceiver", relay=sd2, height=0, channel="filter")
...
vis = graphjs(g, l, vertex.size=0.2, main=as.list(threshold), defer=TRUE, edge.alpha=0.5, deferfps=30,
        crosstalk=sd2, width="100%", height=900)

browsable(div(list(HTML("<center>"), tags$h3("Precision matrix quantile threshold (adjust slider to change)"), slider, vis, relay)))
```
